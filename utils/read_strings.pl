...
print "-- this file is autogenerated by $0 < ./sources/daadario/classical_guitar.csv  > ./db/insert_strings.sql\n";
...
foreach my $manufacturer ( keys %{$db->{manufacturers}} ) {
  ...
  foreach my $part_id ( sort keys %{$db->{manufacturers}->{$manufacturer}->{part_id}} ) {
    ...
    my $description = "$m $part->{string_family} $part->{instrument_category} $part->{instrument_name} String - $part_id - $part->{string_classifcation} - $part->{string_note}($part->{string_order}) - $part->{diameter}\" (" . $part->{diameter}*25.4 . "mm) - $part->{string_tension_category} - $part->{string_material}";

    # Lowercasing strings (yes not great that I uppercase first letter)
    $part->{instrument_category}     = lc($part->{instrument_category});
    $part->{instrument_name}         = lc($part->{instrument_name});
    $part->{string_tension_category} = lc($part->{string_tension_category});
    $part->{string_material}         = lc($part->{string_material});
    $part->{string_classifcation}    = lc($part->{string_classifcation});
